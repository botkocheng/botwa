...
if (!opts['test']) setInterval(() => {
  conn.logger.info('[❗] Scan Qrnya Waktu Cuma 20 Detik subs ary juanda ! !')
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('Subscribe ary juanda')
  else {
...
if (opts['test']) {
  ...
  conn.connect().then(() => {
 conn.sendMessage('6281264038909@c.us', 'Bot Telah Tersambung Ke Database aryganz', 'conversation');
